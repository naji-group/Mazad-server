...
class YTubeController extends Controller
{
    ...
    public function startListener_method( $stream)
    {
      ...
      //  $user_id = auth('api_marketers')->user()->id;
        ...
        // $home = '/var/www/.pm2';
        $listenerPath = base_path("tiktok/ytubelistener.js");
            ...
            $ytService=new  YouTubeCommentsService ();
                ...
                $marketer_social = MarketerSocial::where('marketer_id', $stream->marketer_id)->where('social_id', $social->id)->first();
                 ...
                 if($is_refresh){
                    ...
                    if ($stream->youtube_live_chat_id && $marketer_social->access_token ) {
                      
                         //   \Log::info(" بدء جلب تعليقات يوتيوب الجديدة");
                            $ytComments = $ytService->getNewComments($stream->youtube_live_chat_id,$marketer_social->access_token, $lastYt);
